              ...
              <main className="min-w-0 flex-1 overflow-y-auto">
                {wedding.isLoading && wedding.dataMode === 'supabase' ? (
                  ...
                  <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
                    ...
                    {section === 'households' && <HouseholdManager config={wedding.config} households={wedding.households} selectedIds={activeSelectedIds} onSelectionChange={setSelectedIds} onCreate={async draft => { await wedding.createHousehold(draft); }} onBulkCreate={wedding.bulkCreateHouseholds} onUpdate={wedding.updateHousehold} onDelete={wedding.deleteHousehold} onPreview={(household, variant) => setPreview({ household, variant })} onOpenReport={() => setReportOpen(true)} notify={notify} />}
  ...
  return (
    <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-y-auto bg-gradient-to-br from-[#f8eef1] via-[#fbfaf8] to-[#eef2eb] p-4">
      ...
      <div className="grid w-full max-w-4xl overflow-hidden rounded-[2rem] border border-white/70 bg-white shadow-2xl md:grid-cols-[.9fr_1.1fr]">
        <div className="relative hidden overflow-hidden bg-gradient-to-br from-[#fdf5f8] via-[#fbf0f4] to-[#f8eaef] p-8 text-stone-800 md:flex md:flex-col md:justify-between border-r border-[#f0d5de]">
          <div className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-[#dc8098]/15 blur-2xl" /><div className="absolute -bottom-20 -left-16 h-64 w-64 rounded-full bg-[#e4f0c9]/25 blur-2xl" />
          <div className="relative"><div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-pink-200 bg-white shadow-2xs text-[#8a384b]"><LockKeyhole className="h-5 w-5" /></div><h1 className="mt-6 font-serif text-3xl font-semibold text-stone-900">Everything important, in one private place.</h1><p className="mt-3 text-sm leading-relaxed text-stone-600">Manage guests, access rules, invitations, photos and confirmed site details together.</p></div>
          <div className="relative space-y-3 text-xs text-stone-600"><p className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-[#c97a8b]" /> Approved couple accounts only</p><p className="flex items-center gap-2"><Users className="h-4 w-4 text-[#c97a8b]" /> Private household bearer links</p><p className="flex items-center gap-2"><Gift className="h-4 w-4 text-[#c97a8b]" /> Tag-aware housing and gifts</p></div>
        </div>
