...
import { ArrowDown, CalendarDays, LockKeyhole, MapPin } from 'lucide-react';
...
export function Hero({ onNavigate }: HeroProps) {
  ...
  useEffect(() => {
    ...
    if (isUnlocked && site.weddingDate && !site.dateIsTbc) {
      document.title = `${site.groomName} & ${site.brideName} — ${formattedDate}`;
    } else {
      document.title = `${site.groomName} & ${site.brideName} — Wedding Celebration`;
    }
  }, [isUnlocked, site.groomName, site.brideName, site.weddingDate, site.dateIsTbc, formattedDate]);
    ...
    <section id="home" className="anchor-section relative z-10 min-h-screen min-h-[100svh] overflow-hidden bg-[#1a1c18] pt-[76px] text-white flex flex-col justify-center">
      ...
      <div className="relative mx-auto flex w-full min-h-[calc(100svh-76px)] max-w-[1440px] items-center px-5 py-14 sm:px-8 lg:px-14">
        <div className="max-w-2xl">
          ...
          <h1 className="hero-enter hero-enter-delay font-display text-[clamp(3.2rem,11vw,9.5rem)] font-medium leading-[0.78] tracking-[-0.055em] text-[#fffdf8] drop-shadow-[0_2px_20px_rgba(0,0,0,0.35)] break-words">
            {site.groomName}
          ...
          <div className="hero-enter hero-enter-delay-2 mt-8 grid max-w-xl gap-5 border-y border-white/25 py-5 text-sm sm:grid-cols-2">
            <div
              onClick={!isUnlocked ? () => onNavigate('rsvp') : undefined}
              className={`flex items-start gap-3.5 ${!isUnlocked ? 'cursor-pointer group/date select-none' : ''}`}
              title={!isUnlocked ? 'Click to enter your invitation code and unlock confirmed date & times' : undefined}
            >
              <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white/15 text-[#f8b4c4] backdrop-blur-xs transition-transform group-hover/date:scale-105">
                {isUnlocked ? <CalendarDays className="h-5 w-5" /> : <LockKeyhole className="h-5 w-5" />}
              </div>
              <div>
                <span className="block text-[10px] font-bold uppercase tracking-[0.22em] text-[#f8b4c4]">
                  {isUnlocked ? 'The date' : 'The date & time'}
                </span>
                {isUnlocked ? (
                  <>
                  ...
                  <>
                    <span className="mt-0.5 block font-display text-xl font-medium text-white">Private Celebration</span>
                    <span className="mt-0.5 block text-xs text-[#fce4ec]/85 underline decoration-pink-300/50 underline-offset-2 transition-colors group-hover/date:text-white">
                      Unlock with invitation →
                    </span>
                  </>
                )}
              </div>
            ...
            <div className="flex items-start gap-3.5">
              <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white/15 text-[#f5e6c8] backdrop-blur-xs">
                <MapPin className="h-5 w-5" />
              </div>
              <div>
                <span className="block text-[10px] font-bold uppercase tracking-[0.22em] text-[#f5e6c8]">The place</span>
        ...
        {isUnlocked && daysRemaining !== null && (
          <div className="absolute bottom-24 right-6 hidden text-right text-white/90 md:block lg:bottom-28 lg:right-14 drop-shadow-[0_2px_10px_rgba(0,0,0,0.4)]">
